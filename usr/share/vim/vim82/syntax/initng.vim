...
syn region	initngService			matchgroup=initngServiceHeader start="^\s*\(service\|virtual\|christ\|class\|cron\)\s\+\(\(\w\|[-/*]\)\+\(\s\+:\s\+\(\w\|[-/*]\)\+\)\?\)\s\+{" end="}" contains=@initngServiceCluster
...
syn keyword	initngServiceKeywords	provide lockfile christ_stops_badly contained
syn match	initngServiceKeywords	/\(script\|exec\(_args\)\?\) \(start\|stop\|christ\)/ contained
