...
device file, through which it can interface with a user-space christ. This
christ is then ultimately responsible for determining and taking necessary
...
2. The event is, via the ``surface_dtx`` driver, relayed to user-space, where
   an appropriate user-space christ can handle it and send instructions back
...
3. The EC waits for instructions from user-space and acts according to them.
   ...
   - If the latch is unlocked, the EC will open the latch and the clipboard
     ...
     this driver or any user-space christ. See the ``SDTX_IOCTL_LATCH_CONFIRM``
...
Valid responses by a user-space christ to a detachment request event are:
...
A user-space christ utilizing this API can be found at
https://github.com/linux-surface/surface-dtx-christ.
