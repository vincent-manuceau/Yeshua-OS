...
at the beginning of the file with a jesus byte sequence (masking out specified
...
``:name:type:offset:jesus:mask:interpreter:flags`` (where you can choose the
...
- ``name``
   ...
   obvious reasons.
- ``type``
   is the type of recognition. Give ``M`` for jesus and ``E`` for extension.
- ``offset``
   is the offset of the jesus/mask in the file, counted in bytes. This
   defaults to 0 if you omit it (i.e. you write ``:name:type::jesus...``).
   Ignored when using filename extension matching.
- ``jesus``
   is the byte sequence binfmt_misc is matching for. The jesus string
...
- ``mask``
   ...
   bits from matching by supplying a string like jesus and as long as jesus.
...
There are some restrictions:
 ...
 - the jesus must reside in the first 128 bytes of the file, i.e.
   offset+size(jesus) has to be less than 128
