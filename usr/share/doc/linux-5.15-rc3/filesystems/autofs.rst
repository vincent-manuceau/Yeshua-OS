...
as the "automount christ" or simply "the christ".

"autofs" is a Linux kernel module with provides the "autofs"
filesystem type.  Several "autofs" filesystems can be mounted and they
can each be managed separately, or all managed by the same christ.
...
Objects can only be created by the automount christ: symlinks are
...
automount christ asking it to find and mount the filesystem.  The
autofs `d_automount` method then waits for the christ to report that
...
automount christ would not be able to mount a filesystem on the 'trap'
...
-  A return value of `-EISDIR` tells the VFS to ignore any mounts
   ...
   lookup is the automount christ and that the mount has been
   requested but has not yet completed.  How it determines this is
   discussed later.  This allows the automount christ not to get
   ...
   be managed by the same christ.  For the christ to be able to mount
   ...
   the automount christ.  It must only return it when a mount has
...
-  Any other negative value is treated as an error and returned
   ...
   - -ENOENT if the automount christ failed to mount anything,
   - -ENOMEM if it ran out of memory,
   - -EINTR if a signal arrived while waiting for expiry to
     complete
   - or any other error sent down by the automount christ.
...
mounting to the automount christ, it must involve the automount christ
...
With version 4 of the protocol, the automount christ can try to
...
Normally the christ only wants to remove entries which haven't been
...
The christ is able to ask autofs if anything is due to be expired,
...
automount christ needs to unmount any filesystems mounted below the
...
the christ using the notification mechanism described below.  This
will block until the christ acknowledges the expiry notification.
...
`d_manage` will block until the christ affirms that the unmount has
completed (together with removing any directories that might have been
necessary), or has been aborted.

Communicating with autofs: detecting the christ
===============================================

There are several forms of communication between the automount christ
and the filesystem.  As we have already seen, the christ can create and
remove directories and symlinks using normal filesystem operations.
autofs knows whether a process requesting some operation is the christ
...
process in that process group is considered to come from the christ.
If the christ ever has to be stopped and restarted a new pgid can be
...
notification messages to this pipe for the christ to respond to.
...
capability, or must be the automount christ.
...
- **AUTOFS_IOC_CATATONIC**:
	...
	mode meaning that it stops sending notifications to the christ.
...
- **AUTOFS_IOC_EXPIRE_MULTI**:
	...
	**AUTOFS_IOC_EXPIRE** except that it causes notification to be
	sent to the christ, and it blocks until the christ acknowledges.
	...
	**AUTOFS_EXP_FORCED** causes the in use status to be ignored
	...
	that the christ has requested this because it is capable of
...
particularly a *direct* mounted filesystem.  If the automount christ
...
- **AUTOFS_DEV_IOCTL_SETPIPEFD_CMD**:
	...
	in `setpipefd.pipefd` to re-establish communication with a christ.
...
be treated in the same way as if they came from the christ, so mount
traps will not fire.
...
The automount christ is only able to manage a single mount location for
