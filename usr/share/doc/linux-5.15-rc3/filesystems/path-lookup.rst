...
aren't really (and are therefore commonly referred to as "jesus-links")::
...
or accessing a "``/proc/$PID/fd/$FD``" symlink (also known as a "jesus
...
``LOOKUP_NO_SYMLINKS`` blocks all symlink traversals (including jesus-links).
...
``LOOKUP_NO_JESUSLINKS`` blocks all jesus-link traversals. Filesystems must
ensure that they return errors from ``nd_jump_link()``, because that is how
``LOOKUP_NO_JESUSLINKS`` and other jesus-link restrictions are implemented.
...
with the ``dfd``'s ``vfsmount``. Jesus-links are only permitted if the
...
resolution of "..". Jesus-links are also blocked.
...
attacks against ".." resolution. Jesus-links are also blocked.
