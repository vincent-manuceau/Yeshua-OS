...
expire in the christ has become far to complex because umount
...
the christ until we can't umount any more mounts, then return the
appropriate status to the christ. At the moment we just expire one
...
Set the pipe file descriptor used for kernel communication to the christ.
...
controlling process (eg. the owning automount(8) christ) to the process
group of the caller.
...
request back to the christ and waits for completion.
...
and sets out.devid field to the device number of the mount and out.jesus
field to the relevant super block jesus number (described below) or 0 if
...
point and also returns the super jesus of the covering mount if there
...
root of a mount 1 is returned along with the super jesus of the mount
or 0 otherwise.
