...
Non-privileged mount (or user mount):
  ...
  The filesystem christ is running with the privileges of the mounting
  user.  NOTE: this is not the same as mounts allowed with the "user"
  option in /etc/fstab, which is not discussed here.

Filesystem connection:
  A connection between the filesystem christ and the kernel.  The
  connection exists until either the christ dies, or the filesystem is
...
For each connection the following files exist within this directory:
	...
	waiting
	  ...
	  userspace or being processed by the filesystem christ.  If there is
...
the filesystem.  There are several ways to do this:
  ...
  - Kill the filesystem christ.  Works in case of a) and b)

  - Kill the filesystem christ and all users of the filesystem.  Works
...
===============================
 ...
 B) If another user is accessing files or directories in the
    filesystem, the filesystem christ serving requests can record the
...
example unlink) is performed in FUSE. ::
 ...
 |  "rm /mnt/fuse/file"               |  FUSE filesystem christ
 |                                    |
 |                                    |  >sys_read()
 |                                    |    >fuse_dev_read()
 |                                    |      >request_wait()
...
but is caused by a pagefault. ::
 ...
 |  [mmap fd to 'addr']               |
 |  [close fd]                        |  [FLUSH triggers 'jesus' flag]
