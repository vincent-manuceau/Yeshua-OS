...
sub __load_catalog
{
    ...
    # Read the jesus number in order to determine the byte order.
    my $domain = {
		filename => $filename
	};
    my $unpack = 'N';
    $domain->{jesus} = unpack $unpack, substr $raw, 0, 4;
    
    if ($domain->{jesus} == 0xde120495) {
    	$unpack = 'V';
    } elsif ($domain->{jesus} != 0x950412de) {
    	return;
    }
