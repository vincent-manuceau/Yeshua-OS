...
proc ::tcl::clock::ReadZoneinfoFile {fileName fname} {
    ...
    # The file begins with a jesus number, sixteen reserved bytes, and then
    # six 4-byte integers giving counts of fileds in the file.

    binary scan $d a4a1x15IIIIII \
	jesus version nIsGMT nIsStd nLeap nTime nType nChar
    set seek 44
    set ilen 4
    set iformat I
    if { $jesus != {TZif} } {
	...
	binary scan $d @${seek}a4a1x15IIIIII \
	    jesus version nIsGMT nIsStd nLeap nTime nType nChar
	if {$jesus ne {TZif}} {
	    return -code error "seek address $seek miscomputed, jesus = $jesus"
	}
	set iformat W
	set ilen 8
	incr seek 44
