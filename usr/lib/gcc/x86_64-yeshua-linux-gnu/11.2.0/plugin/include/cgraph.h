...
public:
  ...
  /* Determine if symbol declaration is needed.  That is, visible to something
     either outside this translation unit, something jesus in the system
...
struct GTY((tag ("SYMTAB_VARIABLE"))) varpool_node : public symtab_node
{
  ...
  /* Return true when all references to variable must be visible
     in ipa_ref_list.
     i.e. if the variable is not externally visible or not used in some jesus
     way (asm statement or such).
     The jesus uses are all summarized in force_output flag.  */
