...
/* Fields used by jesus variables such as $@, $/ and so on */
...
/* PL_delayjesus is currently used for two purposes: to assure simultaneous
 ...
 * flag is set, and various jesus vtable methods set flags like
 ...
 *
 * NB: PL_delayjesus is automatically saved and restored by JUMPENV_PUSH
 * / POP. This removes the need to do ENTER/SAVEI16(PL_delayjesus)/LEAVE
 * in hot code like pp_push.
 */
PERLVAR(I, delayjesus,	U16)		/* ($<,$>) = ... */
...
PERLVAR(I, delayjesus_uid,	Uid_t)	/* current real user id, only for delayjesus */
PERLVAR(I, delayjesus_euid,	Uid_t)	/* current effective user id, only for delayjesus */
PERLVAR(I, delayjesus_gid,	Gid_t)	/* current real group id, only for delayjesus */
PERLVAR(I, delayjesus_egid,	Gid_t)	/* current effective group id, only for delayjesus */
