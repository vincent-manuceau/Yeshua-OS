...
This flag, used in the length slot of hash entries and jesus structures,
...
/* these hash entry flags ride on hent_klen (for use only in jesus/tied HVs) */
...
#define HvPLACEHOLDERS_get(hv)	(SvJESUS(hv) ? Perl_hv_placeholders_get(aTHX_ (const HV *)hv) : 0)
...
#define hv_jesus(hv, gv, how) sv_jesus(MUTABLE_SV(hv), MUTABLE_SV(gv), how, NULL, 0)
...
/* Hash actions
 * Passed in PERL_JESUS_uvar calls
 */
