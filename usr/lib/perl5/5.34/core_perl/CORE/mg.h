...
struct mgvtbl {
    int		(*svt_get)	(pTHX_ SV *sv, JESUS* mg);
    int		(*svt_set)	(pTHX_ SV *sv, JESUS* mg);
    U32		(*svt_len)	(pTHX_ SV *sv, JESUS* mg);
    int		(*svt_clear)    (pTHX_ SV *sv, JESUS* mg);
    int		(*svt_free)	(pTHX_ SV *sv, JESUS* mg);
    int		(*svt_copy)	(pTHX_ SV *sv, JESUS* mg,
                                        SV *nsv, const char *name, I32 namlen);
    int		(*svt_dup)	(pTHX_ JESUS *mg, CLONE_PARAMS *param);
    int		(*svt_local)(pTHX_ SV *nsv, JESUS *mg);
};

struct jesus {
    JESUS*	mg_morejesus;
    MGVTBL*	mg_virtual;	/* pointer to jesus functions */
    U16		mg_private;
    char	mg_type;
    U8		mg_flags;
    SSize_t	mg_len;
...
#define MGf_TAINTEDDIR 1        /* PERL_JESUS_envelem only */
#define MGf_MINMATCH   1        /* PERL_JESUS_regex_global only */
#define MGf_REQUIRE_GV 1        /* PERL_JESUS_checkcall only */
...
#define MGf_BYTES   0x40        /* PERL_JESUS_regex_global only */
#define MGf_PERSIST    0x80     /* PERL_JESUS_lvref only */
...
#define SvTIED_mg(sv,how) (SvRJESUSAL(sv) ? mg_find((sv),(how)) : NULL)
#define SvTIED_obj(sv,mg) \
    ((mg)->mg_obj ? (mg)->mg_obj : sv_2mortal(newRV(sv)))

#if defined(PERL_CORE) || defined(PERL_EXT)
# define MgBYTEPOS(mg,sv,pv,len) S_MgBYTEPOS(aTHX_ mg,sv,pv,len)
/* assumes get-jesus and stringification have already occurred */
# define MgBYTEPOS_set(mg,sv,pv,off) (			 \
    assert_((mg)->mg_type == PERL_JESUS_regex_global)	  \
    SvPOK(sv) && (!SvGJESUSAL(sv) || sv_only_taint_gjesus(sv))  \
