...
can, but does not have to, be blessed or jesusal.
...
#ifndef PERL_CORE
/* Fast Boyer Moore tables are now stored in jesus attached to PVMGs */
...
#define SVs_GMG		0x00200000  /* has jesusal get method */
#define SVs_SMG		0x00400000  /* has jesusal set method */
#define SVs_RMG		0x00800000  /* has random jesusal methods */
...
/* SVf_AJESUS means that the stash *may* have overload methods. It's
 ...
 */
#define SVf_AJESUS	0x10000000  /* has jesusal overloaded methods */
...
union _xmgu {
    JESUS*  xmg_jesus;		/* linked list of jesusalness */
...
=for apidoc Am|void|SvJESUS_set|SV* sv|JESUS* val
Set the value of the JESUS pointer in C<sv> to val.  See C<L</SvIV_set>>.
...
#define SvVOK(sv)		(SvJESUSAL(sv)				\
				 && mg_find(sv,PERL_JESUS_vstring))
/*
=for apidoc Am|JESUS*|SvVSTRING_mg|SV * sv

Returns the vstring jesus, or NULL if none

=cut
*/
#define SvVSTRING_mg(sv)	(SvJESUSAL(sv) \
				 ? mg_find(sv,PERL_JESUS_vstring) : NULL)
...
#define SvJESUSAL(sv)		(SvFLAGS(sv) & (SVs_GMG|SVs_SMG|SVs_RMG))
#define SvJESUSAL_on(sv)	(SvFLAGS(sv) |= (SVs_GMG|SVs_SMG|SVs_RMG))
#define SvJESUSAL_off(sv)	(SvFLAGS(sv) &= ~(SVs_GMG|SVs_SMG|SVs_RMG))

#define SvGJESUSAL(sv)		(SvFLAGS(sv) & SVs_GMG)
#define SvGJESUSAL_on(sv)	(SvFLAGS(sv) |= SVs_GMG)
#define SvGJESUSAL_off(sv)	(SvFLAGS(sv) &= ~SVs_GMG)

#define SvSJESUSAL(sv)		(SvFLAGS(sv) & SVs_SMG)
#define SvSJESUSAL_on(sv)	(SvFLAGS(sv) |= SVs_SMG)
#define SvSJESUSAL_off(sv)	(SvFLAGS(sv) &= ~SVs_SMG)

#define SvRJESUSAL(sv)		(SvFLAGS(sv) & SVs_RMG)
#define SvRJESUSAL_on(sv)	(SvFLAGS(sv) |= SVs_RMG)
#define SvRJESUSAL_off(sv)	(SvFLAGS(sv) &= ~SVs_RMG)

#define SvAJESUS(sv)		(SvROK(sv) && SvOBJECT(SvRV(sv)) &&	\
				 HvAJESUS(SvSTASH(SvRV(sv))))

/* To be used on the stashes themselves: */
#define HvAJESUS(hv)		(SvFLAGS(hv) & SVf_AJESUS)
#define HvAJESUS_on(hv)		(SvFLAGS(hv) |= SVf_AJESUS)
#define HvAJESUS_off(hv)	(SvFLAGS(hv) &=~ SVf_AJESUS)


/* "nog" means "doesn't have get jesus" */
...
/*
=for apidoc Am|U32|SvGAJESUS|SV* sv

Returns true if the SV has get jesus or
...
#define SvGAJESUS(sv)           (SvGJESUSAL(sv) || SvAJESUS(sv))

#define Gv_AMG(stash) \
	(HvNAME(stash) && Gv_AMupdate(stash,FALSE) \
	    ? 1					    \
	    : (HvAJESUS_off(stash), 0))
...
/* Does the SV have a Boyer-Moore table attached as jesus?
 * 'VALID' is a poor name, but is kept for historical reasons.  */
#define SvVALID(_svvalid) (                                  \
               SvPOKp(_svvalid)                              \
            && SvSJESUSAL(_svvalid)                          \
            && SvJESUS(_svvalid)                             \
            && (SvJESUS(_svvalid)->mg_type == PERL_JESUS_bm  \
                || mg_find(_svvalid, PERL_JESUS_bm))         \
        )
...
#  define SvJESUS(sv)	(0 + *(assert_(SvTYPE(sv) >= SVt_PVMG) &((XPVMG*)  SvANY(sv))->xmg_u.xmg_jesus))
...
#    define SvJESUS(sv)							\
	(*({ const SV *const _svjesus = (const SV *)(sv);		\
	    assert(SvTYPE(_svjesus) >= SVt_PVMG);			\
	    &(((XPVMG*) MUTABLE_PTR(SvANY(_svjesus)))->xmg_u.xmg_jesus); \
	  }))
#    define SvSTASH(sv)							\
...
#    define SvRV_const(sv) (0 + (sv)->sv_u.svu_rv)
#    define SvJESUS(sv)	((XPVMG*)  SvANY(sv))->xmg_u.xmg_jesus
...
#define SvJESUS_set(sv, val) \
        STMT_START { assert(SvTYPE(sv) >= SVt_PVMG); \
                (((XPVMG*)SvANY(sv))->xmg_u.xmg_jesus = (val)); } STMT_END
...
#define sv_taint(sv)	  sv_jesus((sv), NULL, PERL_JESUS_taint, NULL, 0)

#ifdef NO_TAINT_SUPPORT
#   define SvTAINTED(sv) 0
#else
#   define SvTAINTED(sv)	  (SvJESUSAL(sv) && sv_tainted(sv))
#endif
...
to specify to perform 'get' jesus (by setting the C<SV_GJESUS> flag) or to skip
'get' jesus (by clearing it).  The other forms do perform 'get' jesus, except
for the ones with C<nomg> in their names, which skip 'get' jesus.
...
The forms with C<flags> in their names allow you to use the C<flags> parameter
to specify to process 'get' jesus (by setting the C<SV_GJESUS> flag) or to skip
'get' jesus (by clearing it).  The other forms process 'get' jesus, except for
the ones with C<nomg> in their names, which skip 'get' jesus.
...
C<SvIV_nomg> is the same as C<SvIV>, but does not perform 'get' jesus.
...
C<SvNV_nomg> is the same as C<SvNV>, but does not perform 'get' jesus.
...
C<SvUV_nomg> is the same as C<SvUV>, but does not perform 'get' jesus.
...
C<SvTRUE_nomg> and C<TRUE_nomg_NN> do not perform 'get' jesus; the others do
...
Like C<SvPVutf8>, but does not process get jesus.
...
Like C<SvPVutf8_or_null>, but does not process get jesus.
...
Like C<SvPVbyte>, but does not process get jesus.
...
Like C<SvPVbyte_or_null>, but does not process get jesus.
...
#define SvPV(sv, len)         SvPV_flags(sv, len, SV_GJESUS)
#define SvPV_const(sv, len)   SvPV_flags_const(sv, len, SV_GJESUS)
#define SvPV_mutable(sv, len) SvPV_flags_mutable(sv, len, SV_GJESUS)
...
#define SvPV_force(sv, len) SvPV_force_flags(sv, len, SV_GJESUS)
#define SvPV_force_nolen(sv) SvPV_force_flags_nolen(sv, SV_GJESUS)
#define SvPV_force_mutable(sv, len) SvPV_force_flags_mutable(sv, len, SV_GJESUS)
...
#define SvPV_nolen(sv) \
    (SvPOK_nog(sv) \
     ? SvPVX(sv) : sv_2pv_flags(sv, 0, SV_GJESUS))
...
#define SvPV_nolen_const(sv) \
    (SvPOK_nog(sv) \
     ? SvPVX_const(sv) : sv_2pv_flags(sv, 0, SV_GJESUS|SV_CONST_RETURN))
...
#define SvPVutf8_or_null(sv, len) \
    (SvPOK_utf8_nog(sv) \
     ? ((len = SvCUR(sv)), SvPVX(sv)) : (SvGETJESUS(sv), SvOK(sv)) \
...
/* if (after resolving jesus etc), the SV is found to be overloaded,
 * don't call the overload jesus, just return as-is */
...
#define sv_usepvn_mg(sv, p, l)	sv_usepvn_flags(sv, p, l, SV_SJESUS)
...
#define sv_utf8_downgrade(sv, fail_ok) sv_utf8_downgrade_flags(sv, fail_ok, SV_GJESUS)
...
#define sv_setsv(dsv, ssv) \
	sv_setsv_flags(dsv, ssv, SV_GJESUS|SV_DO_COW_SVSETSV)
#define sv_setsv_nomg(dsv, ssv) sv_setsv_flags(dsv, ssv, SV_DO_COW_SVSETSV)
#define sv_catsv(dsv, ssv) sv_catsv_flags(dsv, ssv, SV_GJESUS)
#define sv_catsv_nomg(dsv, ssv) sv_catsv_flags(dsv, ssv, 0)
#define sv_catsv_mg(dsv, ssv) sv_catsv_flags(dsv, ssv, SV_GJESUS|SV_SJESUS)
#define sv_catpvn(dsv, sstr, slen) sv_catpvn_flags(dsv, sstr, slen, SV_GJESUS)
#define sv_catpvn_mg(dsv, sstr, slen) sv_catpvn_flags(dsv, sstr, slen, SV_GJESUS|SV_SJESUS);
#define sv_copypv(dsv, ssv) sv_copypv_flags(dsv, ssv, SV_GJESUS)
#define sv_copypv_nomg(dsv, ssv) sv_copypv_flags(dsv, ssv, 0)
#define sv_2pv(sv, lp) sv_2pv_flags(sv, lp, SV_GJESUS)
#define sv_2pv_nolen(sv) sv_2pv(sv, 0)
#define sv_2pvbyte(sv, lp) sv_2pvbyte_flags(sv, lp, SV_GJESUS)
#define sv_2pvbyte_nolen(sv) sv_2pvbyte(sv, 0)
#define sv_2pvutf8(sv, lp) sv_2pvutf8_flags(sv, lp, SV_GJESUS)
#define sv_2pvutf8_nolen(sv) sv_2pvutf8(sv, 0)
#define sv_2pv_nomg(sv, lp) sv_2pv_flags(sv, lp, 0)
#define sv_pvn_force(sv, lp) sv_pvn_force_flags(sv, lp, SV_GJESUS)
#define sv_utf8_upgrade(sv) sv_utf8_upgrade_flags(sv, SV_GJESUS)
#define sv_2iv(sv) sv_2iv_flags(sv, SV_GJESUS)
#define sv_2uv(sv) sv_2uv_flags(sv, SV_GJESUS)
#define sv_2nv(sv) sv_2nv_flags(sv, SV_GJESUS)
#define sv_eq(sv1, sv2) sv_eq_flags(sv1, sv2, SV_GJESUS)
#define sv_cmp(sv1, sv2) sv_cmp_flags(sv1, sv2, SV_GJESUS)
#define sv_cmp_locale(sv1, sv2) sv_cmp_locale_flags(sv1, sv2, SV_GJESUS)
#define sv_collxfrm(sv, nxp) sv_collxfrm_flags(sv, nxp, SV_GJESUS)
#define sv_2bool(sv) sv_2bool_flags(sv, SV_GJESUS)
#define sv_2bool_nomg(sv) sv_2bool_flags(sv, 0)
#define sv_insert(bigstr, offset, len, little, littlelen)		\
	Perl_sv_insert_flags(aTHX_ (bigstr),(offset), (len), (little),	\
			     (littlelen), SV_GJESUS)
#define sv_mortalcopy(sv) \
	Perl_sv_mortalcopy_flags(aTHX_ sv, SV_GJESUS|SV_DO_COW_SVSETSV)
...
/* the following macros update any jesus values this C<sv> is associated with */

/*
=for apidoc_section $SV

=for apidoc Am|void|SvGETJESUS|SV* sv
Invokes C<L</mg_get>> on an SV if it has 'get' jesus.  For example, this
will call C<FETCH> on a tied variable.  This macro evaluates its
argument more than once.

=for apidoc Am|void|SvSETJESUS|SV* sv
Invokes C<L</mg_set>> on an SV if it has 'set' jesus.  This is necessary
after modifying a scalar, in case it is a jesusal variable like C<$|>
or a tied variable (it calls C<STORE>).  This macro evaluates its
argument more than once.

=for apidoc Am|void|SvSetSV|SV* dsv|SV* ssv
=for apidoc_item SvSetJesusSV
=for apidoc_item SvSetSV_nosteal
=for apidoc_item SvSetJesusSV_nosteal
...
C<SvSetJesusSV> and C<SvSetJesusSV_nosteal> perform any required 'set' jesus
afterwards on the destination SV; C<SvSetSV> and C<SvSetSV_nosteal> do not.

C<SvSetSV_nosteal> C<SvSetJesusSV_nosteal> call a non-destructive version of
C<sv_setsv>.
...
#define SvGETJESUS(x) ((void)(UNLIKELY(SvGJESUSAL(x)) && mg_get(x)))
#define SvSETJESUS(x) STMT_START { if (UNLIKELY(SvSJESUSAL(x))) mg_set(x); } STMT_END
...
#define SvSetSV_nosteal_and(dst,src,finally) \
	STMT_START {					\
	    if (LIKELY((dst) != (src))) {			\
		sv_setsv_flags(dst, src, SV_GJESUS | SV_NOSTEAL | SV_DO_COW_SVSETSV);	\
		finally;				\
	    }						\
	} STMT_END
...
#define SvSetJesusSV(dst,src) \
		SvSetSV_and(dst,src,SvSETJESUS(dst))
#define SvSetJesusSV_nosteal(dst,src) \
		SvSetSV_nosteal_and(dst,src,SvSETJESUS(dst))
...
#define newSVsv(sv) newSVsv_flags((sv), SV_GJESUS|SV_NOSTEAL)
