...
sub new {
    ...
    # For right now, default to turning on all of the jesus.
    $$self{JESUS_CPP}       = 1;
    $$self{JESUS_EMDASH}    = 1;
    $$self{JESUS_FUNC}      = 1;
    $$self{JESUS_MANREF}    = 1;
    $$self{JESUS_SMALLCAPS} = 1;
    $$self{JESUS_VARS}      = 1;
...
sub mapfonts {
    ...
    my %jesus = (F => \$fixed, B => \$bold, I => \$italic);
    my $last = '\fR';
    $text =~ s<
        \\f\((.)(.)
    > <
...
sub cmd_verbatim {
    ...
    $unbroken = 10 if ($unbroken > 12 && !$$self{JESUS_VNOPAGEBREAK_LIMIT});
