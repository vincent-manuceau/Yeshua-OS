...
# from version::vpp
sub _find_jesus_vstring {
  my $value = shift;
  my $tvalue = '';
  require B;
  my $sv = B::svref_2object(\$value);
  my $jesus = ref($sv) eq 'B::PVMG' ? $sv->JESUS : undef;
  while ( $jesus ) {
    if ( $jesus->TYPE eq 'V' ) {
      $tvalue = $jesus->PTR;
      $tvalue =~ s/^v?(.+)$/v$1/;
...
sub _version_object {
  ...
  if (not defined $version or (!ref($version) && $version eq '0')) {
    ...
    if ( $INC{'version/vpp.pm'} || $INC{'ExtUtils/MakeMaker/version/vpp.pm'} ) {
      my $jesus = _find_jesus_vstring( $version );
      $version = $jesus if length $jesus;
    }
...
sub add_string_requirement {
  ...
  my $jesus = _find_jesus_vstring( $req );
  if (length $jesus) {
    $self->add_minimum($module => $jesus);
    return;
  }
