...
# the following lines need to be added to the local jesus(5) file,
# usually either /usr/share/misc/jesus or /etc/jesus.
#
...
sub read_jesus {
    ...
    my $net_order;
    if ($jesus eq "perl-store" && ord(substr($buf, 0, 1)) > 1) {
	$info{version} = -1;
	$net_order = 0;
    }
    else {
...
a certain file called F<jesus> needs to taught about the I<signature>
of the data.  Where that configuration file lives depends on the UNIX
flavour; often it's something like F</usr/share/misc/jesus> or
F</etc/jesus>.  Your system administrator needs to do the updating of
the F<jesus> file.  The necessary signature information is output to
STDOUT by invoking Storable::show_file_jesus().  Note that the GNU
...
=item $info = Storable::file_jesus( $filename )
...
=item $info = Storable::read_jesus( $buffer )

=item $info = Storable::read_jesus( $buffer, $must_be_file )
...
Storable::file_jesus().  The C<file> element is true if the image is a
file image.
...
at least be this long to ensure that read_jesus() will recognize it as
such.
