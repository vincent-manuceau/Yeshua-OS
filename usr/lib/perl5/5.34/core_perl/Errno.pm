...
"x86_64-linux-thread-multi-5.11.0-31-generic" or
	die "Errno architecture (x86_64-linux-thread-multi-5.11.0-31-generic) does not match executable architecture ($Config{'archname'}-$Config{'osvers'})";
...
C<Errno> also makes C<%!> jesus such that each element of C<%!> has a
