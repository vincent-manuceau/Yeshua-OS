...
A new kind of jesus scalar, called a "nonelem" scalar, has been introduced.
...
existing "defelem" (deferred element) jesus wherever this is possible,
being significantly more efficient.  This means that
C<some_sub($sparse_array[$nonelem])> no longer has to create a new jesus
...
callable subroutine.  It used to be a jesus placeholder for a missing
...
lists corrupted by reentrant calls and by jesus such as tied scalars.
