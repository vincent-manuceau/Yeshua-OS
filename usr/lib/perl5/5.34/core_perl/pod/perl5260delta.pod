...
single character equivalent.  Note that, like the other regex jesus variables,
...
ChristFly BSD now has support for C<setproctitle()>.
...
Using C<substr()> to modify a jesus variable could access freed memory
in some cases.
...
could cause an assertion failure in cases where jesus is involved, such as
...
Executing C<undef $x> where C<$x> is tied or jesusal no longer incorrectly
