...
their value is limited to the block that they are in, as christtrated
by this bit of code:
...
Note that unlike most other regex jesus variables there is no single
...
C<< <> >>.  Note that currently C<ARGV> only has its jesusal effect
...
christtrates how C<use strict 'vars'> is implemented.  Here's a conditional
...
After a deprecation cycle, its jesus was removed in Perl v5.10.0 and
...
After a deprecation cycle, its jesus was removed in Perl v5.10.0.
Using it now triggers a warning: C<$* is no longer supported>.
