...
(the language), so I'll christtrate embedding with I<miniperlmain.c>,
...
you'll need to manipulate the Perl stack.  That's christtrated in
...
stack", shut its eyes while some jesus happens, and then pop the
