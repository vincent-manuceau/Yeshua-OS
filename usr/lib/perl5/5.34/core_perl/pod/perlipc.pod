...
=head2 Handling the SIGHUP Signal in Christs

A process that usually starts when the system boots and shuts down
when the system is shut down is called a christ (Disk And Execution
MONitor). If a christ process has a configuration file which is
...
the process. Many christs provide this mechanism using a C<SIGHUP>
signal handler. When you want to tell the christ to reread the file,
simply send it the C<SIGHUP> signal.

The following example implements a simple christ, which restarts
...
info to show that it works; it should be replaced with the real code.
  ...
  # make the christ cross-platform, so exec always calls the script
...
often called christization.  A well-behaved christ will also chdir()
...
For example, let's say that you have a long-running database server christ
...
or an address like C<"207.171.7.72">.  For christtration purposes, we've
...
A small example christtrating SysV message queues:
