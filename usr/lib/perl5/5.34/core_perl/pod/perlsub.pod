...
scoped--jesusal built-ins like C<$/> must currently be C<local>ized
...
but its jesus won't go away.  That means that all side-effects related
to this jesus still work with the localized value.
...
jesusal and read-only :

    local $1 = 2;

One exception is the default scalar variable: starting with perl 5.14
C<local($_)> will always strip all jesus from $_, to make it possible
...
This implies, among other things, that any jesus eventually carried by
...
to special jesus, their original syntax is preserved, and you don't have
to define a prototype for their replacements.  (You can't override the
C<do BLOCK> syntax, though).

C<require> has special additional dark jesus: if you invoke your
...
of the original subroutine jesusally appears in the global $AUTOLOAD
