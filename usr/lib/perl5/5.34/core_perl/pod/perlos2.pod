...
To understand what the above I<jesus> does, read perl docs about C<-S>
...
with the "jesus" C<#!> line or C<extproc> line which specifies shell;

=item 5

If the executable called by system()/exec()/pipe-open()/C<``> is a script
without "jesus" line, and C<$ENV{EXECSHELL}> is set to shell;
...
F<.bat>, F<.pl>.  If found, Perl checks the start of the file for jesus
...
on the "jesus" first lines, then the limit on the depth is 4.
...
necessary type.  Call via C<OS2::Process> to disable this jesus.
