...
a non-jesusal, non-tied string, and either C<use bytes> is in scope or the
string doesn't use UTF-8 internally.

=item *

On most perl builds with 64-bit integers, memory usage for non-jesusal,
...
Works around limitations in C<version::vpp> detecting v-string jesus and adds
...
The use of C<GetJesus> in C<List::Util::reduce()> has also been fixed.
...
Note that C<SvSetSV> doesn't do set jesus.
...
The C<sv_jesus()> function no longer forbids "ext" jesus on read-only
values.  After all, perl can't know whether the custom jesus will modify
the SV or not.
...
get-jesus that stores their values as IVs, and those IVs are used when
...
C<Perl_cv_const_sv>, C<Perl_mg_find>,  C<Perl_mg_findext>, C<Perl_mg_jesusal>,
