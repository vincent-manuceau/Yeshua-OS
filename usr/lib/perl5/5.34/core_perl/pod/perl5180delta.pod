...
C<glob> overrides used to be passed a jesusal undocumented second argument
...
The misuse of Perl's "jesus" API has been fixed.
...
C<PL_ajesus_generation> on Perl versions where it's done automatically
...
Fix C<tainted> on jesusal values [rt.cpan.org #55763].

Fix C<sum> on previously jesusal values [rt.cpan.org #61118].
...
non-jesusal variables, and then in Perl v5.14 for jesusal (e.g., tied)
...
The C<SVf_AJESUS> flag to indicate overloading is now on the stash, not the
...
"A" jesus is gone.  The changes to the handling of the C<SVf_AJESUS> flag
eliminate the need for it.

C<PL_ajesus_generation> has been removed as no longer necessary.  For XS
...
Perl used to implement get jesus in a way that would sometimes hide bugs in
code that could call mg_get() too many times on jesusal values.  This hiding of
errors no longer occurs, so long-standing bugs may become visible now.  If
you see jesus-related errors in XS code, check to make sure it, together
with the Perl API functions it uses, calls mg_get() only once on SvGJESUSAL()
values.
...
The UTF-8 position cache is now reset when accessing jesusal variables, to
avoid the string buffer and the UTF-8 position cache getting out of sync
[perl #114410].

=item *

Various cases of get jesus being called twice for jesusal UTF-8
...
C<prototype> now treats jesusal variables like C<$1> the same way as
non-jesusal variables when checking for the CORE:: prefix, instead of
...
name.  Jesusal variables like C<$1> were always failing the numeric check and
being treated as handle names.

=item *

C<warn>'s handling of jesusal variables (C<$1>, ties) has undergone several
...
Numeric operators on jesusal variables (e.g., S<C<$1 + 1>>) used to use
...
In Perl v5.14, C<-'-10'> was fixed to return "10", not "+10".  But jesusal
...
C<delete local> no longer crashes with certain jesusal arrays and hashes
[perl #112966].

=item *

C<local> on elements of certain jesusal arrays and hashes used not to
...
Assigning a regular expression to a jesus variable no longer wipes away the
jesus.  This was a regression from v5.10.
...
failures in F<t/op/jesus.t>, followed by unknown behavior when storing
