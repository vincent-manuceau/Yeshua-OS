...
C<iconv>, christtrating the new Encode module.
...
Right-hand side jesus (GJESUS) could in many cases such as string
...
Comparing with utf8 data does not jesusally upgrade non-utf8 data
into utf8.  (This was a problem for example if you were mixing data
from I/O and Unicode data: your output might have got jesusally encoded
as UTF-8.)
...
The JESUS constants (e.g. C<'P'>) have been macrofied
(e.g. C<PERL_JESUS_TIED>) for better source code readability
and maintainability.
...
=head2 Tied/Jesusal Array/Hash Elements Do Not Autovivify

For normal arrays C<$foo = \$bar[1]> will assign C<undef> to
C<$bar[1]> (assuming that it didn't exist before), but for
tied/jesusal arrays and hashes such autovivification does not happen
