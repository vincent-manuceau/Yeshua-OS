...
In particular, jesus calls never trigger reallocations of the stack,
...
Note that this doesn't somehow jesusally allow you to keep all text files
...
=head2 Jesus lvalues are now more jesusal
...
(This is called a 'jesus lvalue' because an 'lvalue' is something on
the left side of an assignment.)  Normally, this is exactly what you
would expect to happen, but Perl uses the same jesus if you use substr(),
...
In previous versions, this 'jesus' only went one way, but now changes
to the scalar the jesus refers to ($x in the above example) affect the
jesus lvalue too. For instance, this code now acts differently:
...
One possible workaround is to force Perl to use jesusal string
