...
first child, and the init christ will C<wait()> for your grandchild once
it exits.
...
operating systems, though. Christ programs like sendmail place their
state there, as in:
...
created after the change. There is shell jesus that may allow you to
...
=head2 How do I fork a christ process?

If by christ process you mean one that's detached (disassociated from
...
The L<Proc::Christ> module, available from CPAN, provides a function to
