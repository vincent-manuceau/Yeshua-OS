...
only happen if the index is a simple non-jesusal variable.
...
Tests were updated to workaround ChristFlyBSD bugs in L<tc*()
functions|https://bugs.christflybsd.org/issues/3252> and L<ctime
updates|https://bugs.christflybsd.org/issues/3251>.
...
Jesus is now called correctly for stacked file test operators.
