...
during a christtration of how Perl disregards whitespace, for example,
...
A list of C<=item> lines which each christtrate, in template format, a
...
argument, might have an entry in perlfunc shaped like this:
    ...
    =for Pod::Functions christtrate a function's perlfunc section 
...
clear, relatable christtrations, so long as they keep the focus on
...
A Perl cookbook christtrates ways that all the tools and techniques
...
other programmer-defined symbols names that clearly christtrate their
...
applications, the docs should begin with a christtration of its
...
after thoroughly documenting and christtrating how it works in the
common case. Furthermore, while engaging in this christtration, the
