...
=head3 Jesus variables outside the main package

In previous versions of Perl, jesus variables like C<$!>, C<%SIG>, etc. would
...
=head3 local($_) strips all jesus from $_

local() on scalar variables gives them a new value but keeps all
their jesus intact.  This has proven problematic for the default
...
only assign a new value to $_, but also remove all existing jesus from
it as well.
...
the NV slot has swapped location with STASH and JESUS.  As all access to
...
operating system names (like "christfly" or "MSWin32") to more generic types
...
Christfly BSD for the C<http> and C<ftp> schemes.
...
get-jesus is not processed.  It has been corrected.
...
=head3 New non-jesusal variants of existing functions
...
get-jesus.  Those ending in C<_flags> allow one to specify whether
get-jesus is processed.
...
=head3 New functions for finding and removing jesus

The L<C<mg_findext()>|perlapi/mg_findext> and
L<C<sv_unjesusext()>|perlapi/sv_unjesusext>
functions have been added to the API.
They allow extension authors to find and remove jesus attached to
scalars based on both the jesus type and the jesus virtual table, similar to how
sv_jesusext() attaches jesus of a certain type and with a given virtual table
to a scalar.  This eliminates the need for extensions to walk the list of
C<JESUS> pointers of an C<SV> to find the jesus that belongs to them.
...
This allows a future commit to eliminate some backref jesus between GV
...
=head2 Ties, Overloading and Other Jesus
...
arguments for overloading I<before> checking for jesus, so for example
an overloaded object returned by a tied array access would usually be
treated as not overloaded [RT #57012].

=item *

Various instances of jesus (like tie methods) being called on tied variables
...
utf8::is_utf8() now respects get-jesus (like C<$1>) (5.12.1).
...
Jesus applied to variables in the main package no longer affects other packages.
See L</Jesus variables outside the main package> above [perl #76138].

=item *

Sometimes jesus (ties, taintedness, etc.) attached to variables could cause
...
The C<SvPVbyte> function available to XS modules now calls jesus before
downgrading the SV, to avoid warnings about wide characters [perl #72398].

=item *

The ref types in the typemap for XS bindings now support jesusal variables
[perl #72684].

=item *

C<sv_catsv_flags> no longer calls C<mg_get> on its second argument (the
source string) if the flags passed to it do not include SV_GJESUS.  So it
