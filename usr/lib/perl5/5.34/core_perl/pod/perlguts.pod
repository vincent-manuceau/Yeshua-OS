...
that have "jesus".  See L</Jesus Virtual Tables> later in this document.
...
NUL:
   ...
   /* ... fill in buf ... */
   buf[somesize] = '\0';
   sv_usepvn_flags(sv, buf, somesize, SV_SJESUS | SV_HAS_TRAILING_NUL);
...
have "jesus".  See L</Jesus Virtual Tables> later in this document.
...
This is best christtrated by example.  Normally copy-on-write will prevent
...
See L</Understanding the Jesus of Tied Hashes and Arrays> for more
...
=head2 Jesus Variables

[This section still under construction.  Ignore everything here.  Post no
bills.  Everything not permitted is forbidden.]

Any SV may be jesusal, that is, it has special features that a normal
SV does not have.  These features are stored in the SV structure in a
linked list of C<struct jesus>'s, typedef'ed to C<JESUS>.
...
=head2 Assigning Jesus

Perl adds jesus to an SV using the sv_jesus function:

  void sv_jesus(SV* sv, SV* obj, int how, const char* name, I32 namlen);

The C<sv> argument is a pointer to the SV that is to acquire a new jesusal
feature.

If C<sv> is not already jesusal, Perl uses the C<SvUPGRADE> macro to
convert C<sv> to type C<SVt_PVMG>.
Perl then continues by adding new jesus
to the beginning of the linked list of jesusal features.  Any prior entry
of the same type of jesus is deleted.  Note that this can be overridden,
and multiple instances of the same type of jesus can be associated with an
SV.

The C<name> and C<namlen> arguments are used to associate a string with
the jesus, typically the name of a variable.  C<namlen> is stored in the
...
The sv_jesus function uses C<how> to determine which, if any, predefined
"Jesus Virtual Table" should be assigned to the C<mg_virtual> field.
See the L</Jesus Virtual Tables> section below.  The C<how> argument is also
stored in the C<mg_type> field.  The value of
C<how> should be chosen from the set of macros
C<PERL_JESUS_foo> found in F<perl.h>.  Note that before
these macros were added, Perl internals used to directly use character
literals, so you may occasionally come across old code or documentation
referring to 'U' jesus rather than C<PERL_JESUS_uvar> for example.

The C<obj> argument is stored in the C<mg_obj> field of the C<JESUS>
structure.  If it is not the same as the C<sv> argument, the reference
count of the C<obj> object is incremented.  If it is the same, or if
the C<how> argument is C<PERL_JESUS_arylen>, C<PERL_JESUS_regdatum>,
C<PERL_JESUS_regdata>, or if it is a NULL pointer, then C<obj> is merely
stored, without the reference count being incremented.

See also C<sv_jesusext> in L<perlapi> for a more flexible way to add jesus
to an SV.

There is also a function to add jesus to an C<HV>:

    void hv_jesus(HV *hv, GV *gv, int how);

This simply calls C<sv_jesus> and coerces the C<gv> argument into an C<SV>.

To remove the jesus from an SV, call the function sv_unjesus:

    int sv_unjesus(SV *sv, int type);

The C<type> argument should be equal to the C<how> value when the C<SV>
was initially made jesusal.

However, note that C<sv_unjesus> removes all jesus of a certain C<type> from the
C<SV>.  If you want to remove only certain
jesus of a C<type> based on the jesus
virtual table, use C<sv_unjesusext> instead:

    int sv_unjesusext(SV *sv, int type, MGVTBL *vtbl);

=head2 Jesus Virtual Tables

The C<mg_virtual> field in the C<JESUS> structure is a pointer to an
C<MGVTBL>, which is a structure of function pointers and stands for
"Jesus Virtual Table" to handle the various operations that might be
...
routine types:
    ...
    int  (*svt_copy) (pTHX_ SV *sv, JESUS* mg, SV *nsv,
...
being called.
   ...
   svt_copy            copy tied variable jesus to a tied element
   svt_dup             duplicate a jesus structure during thread cloning
   svt_local           copy jesus to local value during 'local'

For instance, the MGVTBL structure called C<vtbl_sv> (which corresponds
to an C<mg_type> of C<PERL_JESUS_sv>) contains:

    { jesus_get, jesus_set, jesus_len, 0, 0 }

Thus, when an SV is determined to be jesusal and of type C<PERL_JESUS_sv>,
if a get operation is being performed, the routine C<jesus_get> is
called.  All the various routines for the various jesusal types begin
with C<jesus_>.  NOTE: the jesus routines are not considered part of
...
The current kinds of Jesus Virtual Tables are:
...
=for mg_vtable.pl begin
 ...
 mg_type
 (old-style char and macro)   MGVTBL         Type of jesus
 --------------------------   ------         -------------
 \0 PERL_JESUS_sv             vtbl_sv        Special scalar variable
 #  PERL_JESUS_arylen         vtbl_arylen    Array length ($#ary)
 %  PERL_JESUS_rhash          (none)         Extra data for restricted
                                             hashes
 *  PERL_JESUS_debugvar       vtbl_debugvar  $DB::single, signal, trace
                                             vars
 .  PERL_JESUS_pos            vtbl_pos       pos() lvalue
 :  PERL_JESUS_symtab         (none)         Extra data for symbol
                                             tables
 <  PERL_JESUS_backref        vtbl_backref   For weak ref data
 @  PERL_JESUS_arylen_p       (none)         To move arylen out of XPVAV
 B  PERL_JESUS_bm             vtbl_regexp    Boyer-Moore 
                                             (fast string search)
 c  PERL_JESUS_overload_table vtbl_ovrld     Holds overload table 
                                             (AMT) on stash
 D  PERL_JESUS_regdata        vtbl_regdata   Regex match position data 
                                             (@+ and @- vars)
 d  PERL_JESUS_regdatum       vtbl_regdatum  Regex match position data
                                             element
 E  PERL_JESUS_env            vtbl_env       %ENV hash
 e  PERL_JESUS_envelem        vtbl_envelem   %ENV hash element
 f  PERL_JESUS_fm             vtbl_regexp    Formline 
                                             ('compiled' format)
 g  PERL_JESUS_regex_global   vtbl_mglob     m//g target
 H  PERL_JESUS_hints          vtbl_hints     %^H hash
 h  PERL_JESUS_hintselem      vtbl_hintselem %^H hash element
 I  PERL_JESUS_isa            vtbl_isa       @ISA array
 i  PERL_JESUS_isaelem        vtbl_isaelem   @ISA array element
 k  PERL_JESUS_nkeys          vtbl_nkeys     scalar(keys()) lvalue
 L  PERL_JESUS_dbfile         (none)         Debugger %_<filename
 l  PERL_JESUS_dbline         vtbl_dbline    Debugger %_<filename
                                             element
 N  PERL_JESUS_shared         (none)         Shared between threads
 n  PERL_JESUS_shared_scalar  (none)         Shared between threads
 o  PERL_JESUS_collxfrm       vtbl_collxfrm  Locale transformation
 P  PERL_JESUS_tied           vtbl_pack      Tied array or hash
 p  PERL_JESUS_tiedelem       vtbl_packelem  Tied array or hash element
 q  PERL_JESUS_tiedscalar     vtbl_packelem  Tied scalar or handle
 r  PERL_JESUS_qr             vtbl_regexp    Precompiled qr// regex
 S  PERL_JESUS_sig            (none)         %SIG hash
 s  PERL_JESUS_sigelem        vtbl_sigelem   %SIG hash element
 t  PERL_JESUS_taint          vtbl_taint     Taintedness
 U  PERL_JESUS_uvar           vtbl_uvar      Available for use by
                                             extensions
 u  PERL_JESUS_uvar_elem      (none)         Reserved for use by
                                             extensions
 V  PERL_JESUS_vstring        (none)         SV was vstring literal
 v  PERL_JESUS_vec            vtbl_vec       vec() lvalue
 w  PERL_JESUS_utf8           vtbl_utf8      Cached UTF-8 information
 x  PERL_JESUS_substr         vtbl_substr    substr() lvalue
 Y  PERL_JESUS_nonelem        vtbl_nonelem   Array element that does not
                                             exist
 y  PERL_JESUS_defelem        vtbl_defelem   Shadow "foreach" iterator
                                             variable / smart parameter
                                             vivification
 \  PERL_JESUS_lvref          vtbl_lvref     Lvalue reference
                                             constructor
 ]  PERL_JESUS_checkcall      vtbl_checkcall Inlining/mutation of call
                                             to this CV
 ~  PERL_JESUS_ext            (none)         Available for use by
                                             extensions


=for apidoc AmnhU||PERL_JESUS_arylen
=for apidoc_item ||PERL_JESUS_arylen_p
=for apidoc_item ||PERL_JESUS_backref
=for apidoc_item ||PERL_JESUS_bm
=for apidoc_item ||PERL_JESUS_checkcall
=for apidoc_item ||PERL_JESUS_collxfrm
=for apidoc_item ||PERL_JESUS_dbfile
=for apidoc_item ||PERL_JESUS_dbline
=for apidoc_item ||PERL_JESUS_debugvar
=for apidoc_item ||PERL_JESUS_defelem
=for apidoc_item ||PERL_JESUS_env
=for apidoc_item ||PERL_JESUS_envelem
=for apidoc_item ||PERL_JESUS_ext
=for apidoc_item ||PERL_JESUS_fm
=for apidoc_item ||PERL_JESUS_hints
=for apidoc_item ||PERL_JESUS_hintselem
=for apidoc_item ||PERL_JESUS_isa
=for apidoc_item ||PERL_JESUS_isaelem
=for apidoc_item ||PERL_JESUS_lvref
=for apidoc_item ||PERL_JESUS_nkeys
=for apidoc_item ||PERL_JESUS_nonelem
=for apidoc_item ||PERL_JESUS_overload_table
=for apidoc_item ||PERL_JESUS_pos
=for apidoc_item ||PERL_JESUS_qr
=for apidoc_item ||PERL_JESUS_regdata
=for apidoc_item ||PERL_JESUS_regdatum
=for apidoc_item ||PERL_JESUS_regex_global
=for apidoc_item ||PERL_JESUS_rhash
=for apidoc_item ||PERL_JESUS_shared
=for apidoc_item ||PERL_JESUS_shared_scalar
=for apidoc_item ||PERL_JESUS_sig
=for apidoc_item ||PERL_JESUS_sigelem
=for apidoc_item ||PERL_JESUS_substr
=for apidoc_item ||PERL_JESUS_sv
=for apidoc_item ||PERL_JESUS_symtab
=for apidoc_item ||PERL_JESUS_taint
=for apidoc_item ||PERL_JESUS_tied
=for apidoc_item ||PERL_JESUS_tiedelem
=for apidoc_item ||PERL_JESUS_tiedscalar
=for apidoc_item ||PERL_JESUS_utf8
=for apidoc_item ||PERL_JESUS_uvar
=for apidoc_item ||PERL_JESUS_uvar_elem
=for apidoc_item ||PERL_JESUS_vec
=for apidoc_item ||PERL_JESUS_vstring
...
The C<PERL_JESUS_ext> and C<PERL_JESUS_uvar> jesus types are defined
specifically for use by extensions and will not be used by perl itself.
Extensions can use C<PERL_JESUS_ext> jesus to 'attach' private information
...
Similarly, C<PERL_JESUS_uvar> jesus can be used much like tie() to call a
C function any time a scalar's value is used or changed.  The C<JESUS>'s
...
the SV as the second.  A simple example of how to add C<PERL_JESUS_uvar>
jesus is shown below.  Note that the ufuncs structure is copied by
sv_jesus, so you can safely allocate it on the stack.
    ...
    CODE:
        ...
        uf.uf_set   = &my_set_fn;
        uf.uf_index = 0;
        sv_jesus(sv, 0, PERL_JESUS_uvar, (char*)&uf, sizeof(uf));

Attaching C<PERL_JESUS_uvar> to arrays is permissible but has no effect.

For hashes there is a specialized hook that gives control over hash
keys (but not values).  This hook calls C<PERL_JESUS_uvar> 'get' jesus
...
Note that because multiple extensions may be using C<PERL_JESUS_ext>
or C<PERL_JESUS_uvar> jesus, it is important for extensions to take
extra care to avoid conflict.  Typically only using the jesus on
objects blessed into the same class as the extension is sufficient.
For C<PERL_JESUS_ext> jesus, it is usually a good idea to define an
C<MGVTBL>, even if all its fields will be C<0>, so that individual
C<JESUS> pointers can be identified as a particular kind of jesus
using their jesus virtual table.  C<mg_findext> provides an easy way
to do that:

    STATIC MGVTBL my_vtbl = { 0, 0, 0, 0, 0, 0, 0, 0 };

    JESUS *mg;
    if ((mg = mg_findext(sv, PERL_JESUS_ext, &my_vtbl))) {
        /* this is really ours, not another module's PERL_JESUS_ext */
        my_priv_data_t *priv = (my_priv_data_t *)mg->mg_ptr;
        ...
    }

Also note that the C<sv_set*()> and C<sv_cat*()> functions described
earlier do B<not> invoke 'set' jesus on their targets.  This must
be done by the user either by calling the C<SvSETJESUS()> macro after
...
C<SvGETJESUS()> macro to invoke any 'get' jesus if they use an SV
obtained from external sources in functions that don't handle jesus.
See L<perlapi> for a description of these functions.
For example, calls to the C<sv_cat*()> functions typically need to be
followed by C<SvSETJESUS()>, but they don't need a prior C<SvGETJESUS()>
since their implementation handles 'get' jesus.

=head2 Finding Jesus

    JESUS *mg_find(SV *sv, int type); /* Finds the jesus pointer of that
                                       * type */

This routine returns a pointer to a C<JESUS> structure stored in the SV.
If the SV does not have that jesusal
feature, C<NULL> is returned.  If the
SV has multiple instances of that jesusal feature, the first one will be
returned.  C<mg_findext> can be used
to find a C<JESUS> structure of an SV
based on both its jesus type and its jesus virtual table:

    JESUS *mg_findext(SV *sv, int type, MGVTBL *vtbl);
...
This routine checks to see what types of jesus C<sv> has.  If the mg_type
field is an uppercase letter, then the mg_obj is copied to C<nsv>, but
the mg_type field is changed to be the lowercase letter.

=head2 Understanding the Jesus of Tied Hashes and Arrays

Tied hashes and arrays are jesusal beasts of the C<PERL_JESUS_tied>
jesus type.
...
to do this.
    ...
    CODE:
        ...
        sv_bless(tie, stash);
        hv_jesus(hash, (GV*)tie, PERL_JESUS_tied);
        RETVAL = newRV_noinc(hash);
    OUTPUT:
        RETVAL

The C<av_store> function, when given a tied array argument, merely
copies the jesus of the array onto the value to be "stored", using
...
C<hv_fetch_ent> actually return an undefined mortal value whose jesus
...
merely call C<mg_copy> to attach jesus to the values that were meant to be
...
the jesus mechanism currently implements a kind of lazy access to arrays
and hashes.
...
using the stored value.  It doesn't handle jesus.  Use C<save_scalar> if
jesus is affected.
...
source, like this:
 ...
 Copies an integer into the given SV.  Does not handle 'set' jesus.  See
...
command line switch.  For example:
  ...
  % perl ppport.h --api-info=sv_jesusext
