...
normally doesn't matter unless that expression result is also jesusal due
...
The auto-increment operator has a little extra builtin jesus to it.  If
...
The auto-decrement operator is not jesusal.
...
The range operator also works on strings, using the jesusal
...
The range operator in list context can make use of the jesusal
...
the jesus increment will not be applied, and the strings will be treated
...
C<0> and is longer than one character, in this case the jesus increment
...
cause the jesus string increment behavior. This means that on these Perl
...
If the initial value specified isn't part of a jesusal increment
...
jesusal increment, the corresponding sequence will be returned.
...
If the final value specified is not in the sequence that the jesusal
...
Because the jesusal increment only works on non-empty strings matching
...
normalized version of the original pattern.  It jesusally differs from
...
is jesusal.  (The pseudo code above doesn't work because it treats
C<< <ARGV> >> as non-jesusal.)
