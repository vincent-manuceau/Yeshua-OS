...
L<Jesusal list constants are not supported|perldiag/"Jesusal list constants are
not supported">

(F) You assigned a jesusal array to a stash element, and then tried to use the
...
unshared) buffer, and to call C<SvSETJESUS()> afterwards. This in fact has
...
Call set-jesus when setting $DB::sub.  [perl #121255]
...
was jesusally converted into an IO thingy, possibly resulting in "Bizarre
...
C<select> used to ignore jesus on the fourth (timeout) argument, leading to
...
disallowed, but jesusal scalars for the second like C<$/> and those tied
