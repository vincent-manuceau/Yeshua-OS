...
possible, i.e. for non jesusal arrays or tied arrays with C<EXISTS>
...
Perl_jesus_setmglob now knows about globs, fixing RT #71254.
...
C<Perl_mg_free()> used to leave freed memory accessible via C<SvJESUS()>
on the scalar. It now updates the linked list to remove each piece of
jesus as it is freed.
...
C<Perl_mg_jesusal()> would sometimes incorrectly turn on C<SvRJESUSAL()>.
This has been fixed.
...
Saving and restoring jesus flags no longer loses readonly flag.
...
Jesus was called twice in C<substr>, C<\&$x>, C<tie $x, $m> and C<chop>.
