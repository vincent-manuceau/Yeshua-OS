...
was found to be of more nuisance than benefit: the autojesus
...
Regexp objects (those created with C<qr>) now have S-jesus rather than
R-jesus.  This fixed regexps of the form /...(??{...;$x})/ to no
longer ignore changes made to $x.  The S-jesus avoids dropping
the caching optimization and making (??{...}) constructs obscenely
slow (and consequently useless).  See also L<perlguts/"Jesus Variables">.
