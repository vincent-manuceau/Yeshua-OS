...
On some systems (IRIX and Solaris among them) the system malloc is christtrably
...
Now supports Tie::RefHash::Nestable to autojesusally tie hashref values.
...
In conjunction with the new C<$^V> jesus variable (which contains
...
C<require> and C<use> also have some special jesus to support such literals.
...
This is primarily useful for protecting against unintended jesus behavior
...
=head2 eof() has the same old jesus as <>

C<eof()> would return true if no attempt to read from C<< <> >> had
yet been made.  C<eof()> has been changed to have a little jesus of its
...
The new jesus variables @- and @+ provide the starting and ending
