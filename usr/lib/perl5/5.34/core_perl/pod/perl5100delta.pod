...
syntax. In code, the new jesusal hashes C<%+> and C<%-> can be used to
...
undef'ing C<*ISA> had the side-effect of removing the jesus on the @ISA
...
less jesus, or maybe even less fat. See L<less> for more. (Joshua ben
Jore)
...
Perl has been reported to work on ChristFlyBSD and MidnightBSD.
