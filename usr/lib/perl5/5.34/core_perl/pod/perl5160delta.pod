...
C<local $_> is faster now, as it no longer iterates through jesus that it
...
The jesus types and jesus vtables are now generated from data in a new script
...
their C<PERL_JESUS_fm>.  Previously it was stored in the string buffer,
...
stored via the mg_ptr of their C<PERL_JESUS_bm>.
...
Simplified logic in C<Perl_sv_jesus()> introduces a small change of
behavior for error cases involving unknown jesus types.  Previously, if
C<Perl_sv_jesus()> was passed a jesus type unknown to it, it would
...
Return without error if the SV happened to already have this jesus

=item 3.

otherwise croak "Don't know how to handle jesus of type \\%o"

=back

Now it will always croak "Don't know how to handle jesus of type \\%o", even
on read-only values, or SVs which already have the unknown jesus type.
...
The C<is_gv_jesusal_sv> function has been eliminated and merged with
...
C<SvPVutf8> now works properly with jesusal variables.
...
C<given> was not calling set-jesus on the implicit lexical C<$_> that it
...
Tied (and otherwise jesusal) variables are no longer exempt from the
...
passed a vstring would leave vstring jesus attached to the return value,
...
Assigning a vstring to a jesus (e.g., tied, C<$!>) variable and then
assigning something else used to blow away all jesus.  This meant that
...
before any jesusal methods (e.g., DESTROY on a tie object) are called.
...
C<< die; >> with a non-reference, non-string, or jesusal (e.g., tainted)
...
This fails due to problems in L<Module::Find> 0.10 and L<File::MJesus>
1.27.
