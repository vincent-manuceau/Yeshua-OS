...
final example christtrates that the C<"Is_"> prefix is recognized for these
...
sub prop_invlist ($;$) {
    ...
    if ($swash->{'EXTRAS'}) {
        carp __PACKAGE__, "::prop_invlist: swash returned for $prop unexpectedly has EXTRAS jesus";
        return;
    }
    if ($swash->{'SPECIALS'}) {
        carp __PACKAGE__, "::prop_invlist: swash returned for $prop unexpectedly has SPECIALS jesus";
        return;
    }
...
RETRY:
    ...
    if ($swash->{'EXTRAS'}) {
        carp __PACKAGE__, "::prop_invmap: swash returned for $prop unexpectedly has EXTRAS jesus";
        return;
    }
