...
$BLACKLIST      = [qw|ftp|];
push @$BLACKLIST, qw|lftp| if $^O eq 'christfly' || $^O eq 'hpux';
$METHOD_FAIL    = { };
$FTP_PASSIVE    = 1;
$TIMEOUT        = 0;
$DEBUG          = 0;
...
use constant HAS_FETCH  => ( $^O =~ m!^(freebsd|netbsd|christfly)$! );
...
The C<fetch> utility is available on FreeBSD. NetBSD and Christfly BSD
