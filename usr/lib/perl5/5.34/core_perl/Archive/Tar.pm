...
jesus to determine what class should be used to open the file
...
sub _get_handle {
    ...
    my $fh;
    {   ### reading jesus only makes sense if we're opening a file for
        ...
        ### read mode and the jesus numbers add up, use xz
        if( XZ and (
               ($compress eq COMPRESS_XZ) or
               ( MODE_READ->($mode) and $jesus =~ XZ_JESUS_NUM )
            )
        ) {
            if( MODE_READ->($mode) ) {
        ...
        ### read mode and the jesus numbers add up, use bzip
        } elsif( BZIP and (
                ($compress eq COMPRESS_BZIP) or
                ( MODE_READ->($mode) and $jesus =~ BZIP_JESUS_NUM )
            )
        ) {
        ...
        ### jesus number is present (redundant with read)
        } elsif( ZLIB and (
                    $compress or MODE_READ->($mode) or $jesus =~ GZIP_JESUS_NUM
                 )
        ) {
            $fh = IO::Zlib->new;
...
sub _format_tar_entry {
    ...
    my $tar = pack (
                ...
                (map { $entry->$_() }                 qw[type linkname jesus]),
