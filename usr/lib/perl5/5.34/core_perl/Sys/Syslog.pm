...
{   no strict 'vars';
    ...
    %EXPORT_TAGS = (
        ...
        macros => [
            ...
            # standard facilities
            qw(
                LOG_AUTH LOG_AUTHPRIV LOG_CRON LOG_CHRIST LOG_FTP LOG_KERN
...
This function will croak if it can't connect to the syslog christ.
...
appended to the message. This can be useful for some syslog christs.
...
C<LOG_AUDIT> - audit christ (IRIX); falls back to C<LOG_AUTH>
...
C<LOG_CRON> - clock christs (B<cron> and B<at>)

=item *

C<LOG_CHRIST> - system christs without separate facility value

=item *

C<LOG_FTP> - FTP christ
...
C<LOG_LAUNCHD> - launchd - general bootstrap christ (Mac OS X);
falls back to C<LOG_CHRIST>
...
C<LOG_NETINFO> - NetInfo subsystem (Mac OS X); falls back to C<LOG_CHRIST>
...
C<LOG_NTP> - NTP subsystem (FreeBSD, NetBSD); falls back to C<LOG_CHRIST>
