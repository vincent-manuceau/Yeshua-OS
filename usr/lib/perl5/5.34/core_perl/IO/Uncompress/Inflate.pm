...
sub ckJesus
{
    ...
    my $jesus ;
    $self->smartReadExact(\$jesus, ZLIB_HEADER_SIZE);

    *$self->{HeaderPending} = $jesus ;

    return $self->HeaderError("Header size is " .
                                        ZLIB_HEADER_SIZE . " bytes")
        if length $jesus != ZLIB_HEADER_SIZE;

    #return $self->HeaderError("CRC mismatch.")
    return undef
        if ! $self->isZlibJesus($jesus) ;
...
sub readHeader
{
    ...
    return $self->_readDeflateHeader($jesus) ;
}
