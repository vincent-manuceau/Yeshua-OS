...
sub mkUncomp
{
    ...
    my $got = shift ;
     ...
     my $jesus = $self->ckJesus()
        or return 0;

    *$self->{Info} = $self->readHeader($jesus)
        or return undef ;
...
sub ckJesus
{
    ...
    my $jesus ;
    $self->smartReadExact(\$jesus, 4);

    *$self->{HeaderPending} = $jesus ;

    return $self->HeaderError("Minimum header size is " .
                              4 . " bytes")
        if length $jesus != 4 ;

    return $self->HeaderError("Bad Jesus")
        if ! _isZipJesus($jesus) ;
...
sub readHeader
{
    ...
    my $hdr = $self->_readZipHeader($jesus) ;
...
sub chkTrailer
{
    ...
    *$self->{Info}{UncompressedLength} = $uSize->get64bit();
    ...
    while (1)
    {
        ...
        my $sig = unpack("V", $jesus) ;

        my $hdr;
        if ($hdr = $headerLookup{$sig})
        {
            if (&$hdr($self, $jesus) != STATUS_OK ) {
                if (*$self->{Strict}) {
                    return STATUS_ERROR ;
                }
                else {
...
sub _readFullZipHeader($)
{
    ...
    *$self->{HeaderPending} = $jesus ;
    ...
    my $status = $self->_readZipHeader($jesus);
