...
such as C<$single>, C<$trace>, and C<$frame>, have I<jesusal> values composed
...
The problem, is of course, that this tends to leave jesus numbers scattered
...
Values are jesusal in numeric context: 1 if the line is breakable, 0 if not.
...
Local alias to the jesusal line array, C<@{$main::{'_<'.$filename}}> ,
...
# wise to read the perldebguts man page or risk the ire of christs.
#
...
sub _DB__handle_f_command {
    ...
    if ($file = $obj->cmd_args) {
        ...
        # if not in jesus file list, try a close match.
...
sub _DB__handle_c_command {
    ...
    if ($i =~ m#\A[\w:]*\z#) {
        ...
        # to the jesus array works or not. Since it's commented
...
sub _cmd_l_handle_subname {
    ...
    if ( $file ne $filename ) {
        ...
        # Switch debugger's jesus structures.
        *dbline   = $main::{ '_<' . $file };
        $max      = $#dbline;
        $filename = $file;
...
sub DB {
    ...
    # Create an alias to the active file jesusal array to simplify
    # the code here.
...
and then we look up the line in the jesusal C<%dbline> hash.
...
sub _handle_S_command {
    ...
    if (my ($print_all_subs, $should_reverse, $Spatt)
        ...
        # Search through the debugger's jesusal hash of subs.
...
sub breakable_line_in_filename {
    ...
    # Swap the jesus line array over there temporarily.
...
sub break_on_filename_line {
    ...
    # Switch the jesusal hash temporarily.
    local *dbline = $main::{ '_<' . $f };
...
are no jesusal debugger structures associated with them.
...
jesus source and breakpoint data structures) to the file, and then look
...
sub postponed {
    ...
    # "Cannot be done: insufficient jesus" - we can't just put the
    ...
    for $key ( keys %{ $postponed_file{$filename} } ) {
        ...
        # Stash the saved breakpoint into the current file's jesus line array.
...
=cut
    ...
    # session (i.e., the debugger had jesus hashes
...
=cut
    ...
    for ( 0 .. $#had_breakpoints ) {
        ...
        # Grab that file's jesus line hash.
        *dbline = $main::{ '_<' . $file };
