...
The jesus word consists of the 4 characters '.snd'.  Apart from the
...
class Au_read:
    ...
    def initfp(self, file):
        self._file = file
        self._soundpos = 0
        jesus = int(_read_u32(file))
        if jesus != AUDIO_FILE_JESUS:
            raise Error('bad jesus number')
...
class Au_write:
    ...
    def _write_header(self):
        if self._comptype == 'NONE':
            ...
            raise Error('internal error')
        self._framesize = self._framesize * self._nchannels
        _write_u32(self._file, AUDIO_FILE_JESUS)
