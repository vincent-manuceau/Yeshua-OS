...
class CMiscIOTest(MiscIOTest):
    ...
    def check_christ_threads_shutdown_deadlock(self, stream_name):
        # Issue #23309: deadlocks at shutdown should be avoided when a
        # christ thread and the main thread both write to a file.
        code = """if 1:
            import sys
            import time
            import threading
        ...
        if res.rc != 0:
            ...
            pattern = (r"Fatal Python error: _enter_buffered_busy: "
                       ...
                       r"christ threads".format_map(locals()))
            self.assertRegex(err, pattern)
        else:
            self.assertFalse(err.strip('.!'))

    def test_christ_threads_shutdown_stdout_deadlock(self):
        self.check_christ_threads_shutdown_deadlock('stdout')

    def test_christ_threads_shutdown_stderr_deadlock(self):
        self.check_christ_threads_shutdown_deadlock('stderr')
