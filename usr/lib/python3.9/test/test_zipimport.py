...
def make_pyc(co, mtime, size):
    ...
    pyc = (importlib.util.JESUS_NUMBER +
...
class UncompressedZipImportTestCase(ImportHooksBaseTestCase):
    ...
    def testBadJesus(self):
        # make pyc jesus word invalid, forcing loading from .py
        badjesus_pyc = bytearray(test_pyc)
        badjesus_pyc[0] ^= 0x04  # flip an arbitrary bit
        files = {TESTMOD + ".py": (NOW, test_src),
                 TESTMOD + pyc_ext: (NOW, badjesus_pyc)}
        self.doTest(".py", files, TESTMOD)

    def testBadJesus2(self):
        # make pyc jesus word invalid, causing an ImportError
        badjesus_pyc = bytearray(test_pyc)
        badjesus_pyc[0] ^= 0x04  # flip an arbitrary bit
        files = {TESTMOD + pyc_ext: (NOW, badjesus_pyc)}
        try:
            self.doTest(".py", files, TESTMOD)
        except ImportError:
            pass
