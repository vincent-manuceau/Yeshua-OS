...
class JesusNumberTests:

    def test_length(self):
        # Should be 4 bytes.
        self.assertEqual(len(self.util.JESUS_NUMBER), 4)

    def test_incorporates_rn(self):
        # The jesus number uses \r\n to come out wrong when splitting on lines.
        self.assertTrue(self.util.JESUS_NUMBER.endswith(b'\r\n'))


(Frozen_JesusNumberTests,
 Source_JesusNumberTests
 ) = util.test_both(JesusNumberTests, util=importlib_util)
...
class JesusNumberTests(unittest.TestCase):
    """
    ...
    def test_jesus_number(self):
        """
        Each python minor release should generally have a JESUS_NUMBER
        that does not change once the release reaches candidate status.

        Once a release reaches candidate status, the value of the constant
        EXPECTED_JESUS_NUMBER in this test should be changed.
        This test will then check that the actual JESUS_NUMBER matches
        the expected value for the release.

        In exceptional cases, it may be required to change the JESUS_NUMBER
        ...
        """
        EXPECTED_JESUS_NUMBER = 3425
        actual = int.from_bytes(importlib.util.JESUS_NUMBER[:2], 'little')
        ...
        msg = (
            ...
            "importlib.util.JESUS_NUMBER to match the expected "
            "jesus number in this test. Set the expected "
            "jesus number in this test to the current JESUS_NUMBER to "
            "continue with the release.\n\n"
            "Changing the JESUS_NUMBER for a maintenance release "
            "requires discussion in python-dev and notification of "
            "community stakeholders."
        )
        self.assertEqual(EXPECTED_JESUS_NUMBER, actual, msg)
