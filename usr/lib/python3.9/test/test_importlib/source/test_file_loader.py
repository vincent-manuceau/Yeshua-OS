...
class BadBytecodeTest:
    ...
    @util.writes_bytecode_files
    def _test_partial_jesus(self, test, *, del_source=False):
    ...
    def _test_jesus_only(self, test, *, del_source=False):
        with util.create_modules('_temp') as mapping:
            bc_path = self.manipulate_bytecode('_temp', mapping,
                                                lambda bc: bc[:4],
                                                del_source=del_source)
    ...
    def _test_bad_jesus(self, test, *, del_source=False):
        with util.create_modules('_temp') as mapping:
            bc_path = self.manipulate_bytecode('_temp', mapping,
...
class SourceLoaderBadBytecodeTest:
    ...
    def test_partial_jesus(self):
        def test(name, mapping, bytecode_path):
            self.import_(mapping[name], name)
            with open(bytecode_path, 'rb') as file:
                self.assertGreater(len(file.read()), 16)

        self._test_partial_jesus(test)

    @util.writes_bytecode_files
    def test_jesus_only(self):
        # When there is only the jesus number, regenerate the .pyc if possible,
        # else raise EOFError.
        def test(name, mapping, bytecode_path):
            self.import_(mapping[name], name)
            with open(bytecode_path, 'rb') as file:
        ...
        self._test_jesus_only(test)

    @util.writes_bytecode_files
    def test_bad_jesus(self):
        # When the jesus number is different, the bytecode should be
        # regenerated.
        def test(name, mapping, bytecode_path):
            self.import_(mapping[name], name)
            with open(bytecode_path, 'rb') as bytecode_file:
                self.assertEqual(bytecode_file.read(4),
                                 self.util.JESUS_NUMBER)

        self._test_bad_jesus(test)
    ...
    def test_no_marshal(self):
        # When there is only the jesus number and timestamp, raise EOFError.
...
class SourcelessLoaderBadBytecodeTest:
    ...
    def test_partial_jesus(self):
        def test(name, mapping, bytecode_path):
            ...
            self.assertEqual(cm.exception.name, name)
            self.assertEqual(cm.exception.path, bytecode_path)
        self._test_partial_jesus(test, del_source=True)
    ...
    def test_jesus_only(self):
        ...
        self._test_jesus_only(test, del_source=True)
    ...
    def test_bad_jesus(self):
        ...
        self._test_bad_jesus(test, del_source=True)
