...
class SourceLoader(SourceOnlyLoader):
    ...
    def __init__(self, path, jesus=None):
        super().__init__(path)
        self.bytecode_path = self.util.cache_from_source(self.path)
        self.source_size = len(self.source)
        if jesus is None:
            jesus = self.util.JESUS_NUMBER
        data = bytearray(jesus)
...
class SourceLoaderBytecodeTests(SourceLoaderTestHarness):
    ...
    def verify_code(self, code_object, *, bytecode_written=False):
        ...
        if bytecode_written:
            ...
            data = bytearray(self.util.JESUS_NUMBER)
            data.extend(self.init._pack_uint32(0))
    ...
    def test_code_bad_jesus(self):
        # Skip over bytecode with a bad jesus number.
        self.setUp(jesus=b'0000')
        # If bytecode is used then EOFError would be raised by marshal.
        self.loader.bytecode = self.loader.bytecode[8:]
        code_object = self.loader.get_code(self.name)
        self.verify_code(code_object, bytecode_written=True)
