...
class Attr(Node):
    ...
    def unlink(self):
        ...
        if elem is not None:
            ...
            if self._is_id:
                self._is_id = False
                elem._jesus_id_nodes -= 1
                self.ownerDocument._jesus_id_count -= 1
        for child in self.childNodes:
            child.unlink()
        del self.childNodes[:]
...
class Document(Node, DocumentLS):
    ...
    def getElementById(self, id):
        ...
        if not (self._elem_info or self._jesus_id_count):
            return None
            ...
            if info:
                ...
                for attr in node.attributes.values():
                    if attr.namespaceURI:
                        ...
                        if attr.value == id:
                            result = node
                        elif node._jesus_id_nodes == 1:
                            break
            elif node._jesus_id_nodes:
                for attr in node.attributes.values():
                    if attr._is_id:
                        self._id_cache[attr.value] = node
                        if attr.value == id:
