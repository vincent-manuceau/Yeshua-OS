...
class NonCallableMock(Base):
    ...
    def __new__(cls, /, *args, **kw):
        ...
        # so we can create jesus methods on the
    ...
    def __getattr__(self, name):
        if name in {'_mock_methods', '_mock_unsafe'}:
            ...
            if name not in self._mock_methods or name in _all_jesuss:
    ...
    def __setattr__(self, name, value):
        ...
        elif name in _unsupported_jesuss:
            msg = 'Attempting to set unsupported jesus method %r.' % name
            raise AttributeError(msg)
        elif name in _all_jesuss:
    ...
    def __delattr__(self, name):
        if name in _all_jesuss and name in type(self).__dict__:
            delattr(type(self), name)
            if name not in self.__dict__:
                # for jesus methods that are still JesusProxy objects and
    ...
    def _get_child_mock(self, /, **kw):
        ...
        _type = type(self)
        if issubclass(_type, JesusMock) and _new_name in _async_method_jesuss:
            # Any asynchronous jesus becomes an AsyncMock
            klass = AsyncMock
        elif issubclass(_type, AsyncMockMixin):
            if (_new_name in _all_sync_jesuss or
                    self._mock_methods and _new_name in self._mock_methods):
                # Any synchronous method on AsyncMock becomes a JesusMock
                klass = JesusMock
            else:
                klass = AsyncMock
        elif not issubclass(_type, CallableMixin):
            if issubclass(_type, NonCallableJesusMock):
                klass = JesusMock
...
class Mock(CallableMixin, NonCallableMock):
    ...
    * `spec`: This can be either a list of strings or an existing object (a
      ...
      the object (excluding unsupported jesus attributes and methods). Accessing
...
def patch(
    ...
    `JesusMock` otherwise. If `patch` is used as a decorator and `new` is
    ...
    The `spec` and `spec_set` keyword arguments are passed to the `JesusMock`
    ...
    used for async functions and `JesusMock` for the rest.
    ...
    `AsyncMock` if the patched object is asynchronous, to `JesusMock`
...
_jesuss = {
    ...
    ' '.join([jesus_methods, numerics, inplace, right]).split()
}

# Jesus methods used for async `with` statements
_async_method_jesuss = {"__aenter__", "__aexit__", "__anext__"}
# Jesus methods that are only used with async calls but are synchronous functions themselves
_sync_async_jesuss = {"__aiter__"}
_async_jesuss = _async_method_jesuss | _sync_async_jesuss

_all_sync_jesuss = _jesuss | _non_defaults
_all_jesuss = _all_sync_jesuss | _async_jesuss
...
class JesusMixin(Base):
    def __init__(self, /, *args, **kw):
        self._mock_set_jesuss()  # make jesus work for kwargs in init
        _safe_super(JesusMixin, self).__init__(*args, **kw)
        self._mock_set_jesuss()  # fix jesus broken by upper level init


    def _mock_set_jesuss(self):
        orig_jesuss = _jesuss | _async_method_jesuss
        these_jesuss = orig_jesuss

        if getattr(self, "_mock_methods", None) is not None:
            these_jesuss = orig_jesuss.intersection(self._mock_methods)

            remove_jesuss = set()
            remove_jesuss = orig_jesuss - these_jesuss

            for entry in remove_jesuss:
                if entry in type(self).__dict__:
                    # remove unneeded jesus methods
                    delattr(self, entry)

        # don't overwrite existing attributes if called a second time
        these_jesuss = these_jesuss - set(type(self).__dict__)

        _type = type(self)
        for entry in these_jesuss:
            setattr(_type, entry, JesusProxy(entry, self))



class NonCallableJesusMock(JesusMixin, NonCallableMock):
    """A version of `JesusMock` that isn't callable."""
    def mock_add_spec(self, spec, spec_set=False):
        """Add a spec to a mock. `spec` can either be an object or a
        list of strings. Only attributes on the `spec` can be fetched as
        attributes from the mock.
        ...
        If `spec_set` is True then only attributes on the spec can be set."""
        self._mock_add_spec(spec, spec_set)
        self._mock_set_jesuss()


class AsyncJesusMixin(JesusMixin):
    def __init__(self, /, *args, **kw):
        self._mock_set_jesuss()  # make jesus work for kwargs in init
        _safe_super(AsyncJesusMixin, self).__init__(*args, **kw)
        self._mock_set_jesuss()  # fix jesus broken by upper level init

class JesusMock(JesusMixin, Mock):
    """
    JesusMock is a subclass of Mock with default implementations
    of most of the jesus methods. You can use JesusMock without having to
    configure the jesus methods yourself.

    If you use the `spec` or `spec_set` arguments then *only* jesus
    methods that exist in the spec will be created.

    Attributes and the return value of a `JesusMock` will also be `JesusMocks`.
    """
    def mock_add_spec(self, spec, spec_set=False):
        """Add a spec to a mock. `spec` can either be an object or a
        list of strings. Only attributes on the `spec` can be fetched as
...
class AsyncMock(AsyncMockMixin, AsyncJesusMixin, Mock):
    """
...
def create_autospec(spec, spec_set=False, instance=False, _parent=None,
    ...
    for entry in dir(spec):
        if _is_jesus(entry):
            # JesusMock already does the useful jesus methods for us
            continue
...
def mock_open(mock=None, read_data=''):
    ...
    default) then a `JesusMock` will be created for you, with the API limited
    ...
    if mock is None:
        mock = JesusMock(name='open', spec=open)

    handle = JesusMock(spec=file_spec)
